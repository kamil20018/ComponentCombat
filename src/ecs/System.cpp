...
void System::drawComponents(sf::Vector2i mousePos) {
  for (auto &[entityID, position] :
    ...
    if (scene->entityHasComponent<component::Size>(entityID)) {
      ...
      if (position->x * TILE_SIZE < mousePos.x &&
          mousePos.x < position->x * TILE_SIZE + size->width &&
          position->y * TILE_SIZE < mousePos.y &&
          mousePos.y < position->y * TILE_SIZE + size->height) {
