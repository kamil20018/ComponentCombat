...
#define iter(name, compType) \
  for (auto &[name##ID, name##Component] : scene->getComponents<compType>())
#define if_has(entity, compType) \
  if (scene->entityHasComponent<compType>(entity))
...
void System::applyPoison() {
  ...
  iter(poisoned, component::Poisoned) {
    if_has(poisonedID, component::Hp) {
      std::cout << poisonedID << " has Poisoned Component, damage: "
                << poisonedComponent->damage
                << " duration: " << poisonedComponent->duration << std::endl;
      scene->getComponent<component::Hp>(poisonedID)->hp -=
          poisonedComponent->damage;
    }
...
void System::drawComponents(sf::Vector2i mousePos) {
  for (auto &[entityID, position] :
       scene->getComponents<component::Position>()) {
    if (scene->entityHasComponent<component::Size>(entityID)) {
      auto size = scene->getComponent<component::Size>(entityID);
      if (position->pos.x * TILE_SIZE < mousePos.x &&
          mousePos.x < position->pos.x * TILE_SIZE + size->width &&
          position->pos.y * TILE_SIZE < mousePos.y &&
...
void System::drawEntities(std::shared_ptr<sf::RenderWindow> window) {
  for (auto &[entityID, bodyColor] :
       scene->getComponents<component::BodyColor>()) {
    if (scene->entityHasComponent<component::Position>(entityID) &&
        scene->entityHasComponent<component::Size>(entityID)) {
      ...
      rectangle.setPosition(position->pos.x * TILE_SIZE,
                            position->pos.y * TILE_SIZE);
